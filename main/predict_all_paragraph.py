...
tokenizer = AutoTokenizer.from_pretrained(
    r"D:\MRC_Competition_Dureader-master\main\models\mrc_roberta_wwm_ext_large")
...
def question_to_target(context, tar):
    children_list = []
    if not len(context) > 0 or not len(tar) > 0:
        return
    if len(tar) > 0:
        flag = True
        child_question = "满足'" + tar + "'有什么前提？"
        while flag:
            ...
            if child_dict['score'] >= 0 and len(child_dict['answer']) > 0 and child_dict['answer'] not in appeared:
                ...
                child_question = '满足' + tar + '中除了' + child_dict['answer'] + '的下一个前提是什么?'
        ...
        if len(children_list) >= 2:
            ...
            temp = QA({'question': children_list[0]['value'] + '和' + children_list[1]['value'] + '是并且还是或者的关系?',
                       'context': context})
            if temp['score'] >= 0:
                relation = temp['answer']
        ans.append({'parent': tar, 'child': children_list,'relation':relation})
        for children in children_list:
            question_to_target(context, children['value'])
...
def question_to_context(context):
    ...
    if len(parent) > 0:
        ...
        question_to_target(context, parent)
    #     flag = True
...
if __name__ == '__main__':
    ...
    with open(r'./test3.csv', 'w', encoding='utf-8', newline='') as f:
        writer = csv.writer(f)
        header = ['parent', 'child', 'child_type', 'relation', 'context']
        writer.writerow(header)
        data = question_to_context(paragraph)
        for temp in data:
            print(temp)
            for child in temp['child']:
                print(child)
                row = [temp['parent'], child['value'], 'ChildTarget', temp['relation'], child['context']]
                writer.writerow(row)
    f.close()
